...
class AcConstants:
    ...
    def __init__(self):
        ...
        self.aircraft_config = aircraft_configs[1] #SELECT BASE AIRCRAFT CONFIGURATION
    ...
    def load_aircraft_data(self):
        ...
        if self.aircraft_config ==  'SR22':
            ...
            # Aircraft reference
            #Factor, wing start weight, wing_loading_ref,
            self.factor = 1.0
            self.mass_total_ref = 1633 * self.factor # 1633[kg] ref cr s22
            self.mass_empty_ref = 1067.8 * self.factor # [kg] from reference
            self.mass_wing_ref = 156.4 #131.2 #171.2#164 #148.5#140.1#156.4 #CHANGE  [kg] 20% from empty weight
            self.mass_body = self.mass_total_ref - self.mass_wing_ref
            self.mass_fuel = 220 * self.factor
            self.load_factor = 3.8 * 1.5
            self.wing_volume_ref = 1.90 #CHANGE
            self.wing_loading_ref =  136 #82, 95, 109, 123,136,150,164
            ...
            self.mass_wing_ref = 234.5  # [kg] 20% from empty weight
            self.mass_body = self.mass_total_ref - self.mass_wing_ref
            self.mass_fuel = 148
            self.load_factor = 3.8 * 1.5
            self.wing_volume_ref = 1.67
            self.wing_loading_ref = 137
            self.correction_factor_raymer =1 #1.2364
            self.transfer_learning = 1
            ...
            self.mass_wing_ref = 1706.8 #20% of empty weight
            self.mass_body = self.mass_total_ref - self.mass_wing_ref
            ...
            self.correction_factor_raymer = 1#1.472966
            self.transfer_learning = 1

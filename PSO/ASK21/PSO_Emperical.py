...
if __name__ == '__main__':
    ...
    optimizer.max_iterations = 100  # optional maximum allowed number of method iterations; when surpassed, optimization is stopped (if reached before other stopping conditions are satistifed)
    ...
    optimizer.target_fitness = -28 # optional fitness threshold; when reached, optimization is stopped (if it didn't already stop due to exhausted pso.max_iterations or pso.maximum_evaluations)
    optimizer.monitoring = 'basic'
    optimizer.convergence_log_file  = 'Results/' + file_name
    # population, generation, fitness_criterion = 20, 40,  0.01

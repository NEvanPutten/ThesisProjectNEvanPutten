...
def objective(design_vector):
    ...
    constr1 = (yoffset_kink-(AC.fuselage_width/2)) - kink_ub * 0.5 * (wing_span - AC.fuselage_width)      #Wing kink location max at half of wing span
    constr2 = kink_lb * 0.5 * (wing_span - AC.fuselage_width) - (yoffset_kink - (AC.fuselage_width/2))
    constr3 = chord_kink - chord_root
    constr4 = chord_tip - chord_kink
    constr5 = chord_kink_lb - chord_kink

    # Geometry check for feasable wing and exit optimization
    if yoffset_kink - (AC.fuselage_width / 2) > kink_ub * 0.5 * (
            wing_span - AC.fuselage_width):
        return -5e-4, constr1, constr2, constr3, constr4, constr5, 0, 0, 0, 0, 0
    #
    if yoffset_kink - (AC.fuselage_width / 2) < kink_lb * 0.5 * (
            wing_span - AC.fuselage_width):
        return -6e-4, constr1, constr2, constr3, constr4, constr5, 0, 0, 0, 0, 0
    #
    if chord_root < chord_kink or chord_kink < chord_tip:
        return -7e-4, constr1, constr2, constr3, constr4, constr5, 0, 0, 0, 0, 0
    #
    if chord_kink < chord_kink_lb:
        return -8e-4, constr1, constr2, constr3, constr4, constr5, 0, 0, 0, 0, 0
    ...
    # Wing mass and volume constraints, continue calculating performance to enhance optimization
    constr6 = wing_mass_percentage - wing_mass_ub
    constr7 = wing_mass_lb - wing_mass_percentage
    constr8 = wing_volume_product - wing_volume
    constr9 = wing_loading - AC.wing_loading_ref * 1.1
    ...
    if alpha_cruise == -2:
        return -1e-8, constr1, constr2, constr3, constr4, constr5, constr6, constr7, constr8, constr9 # AVL failed
    if CL ==  -2:
        return -1e-8, constr1, constr2, constr3, constr4, constr5, constr6, constr7, constr8, constr9 # AVL failed
    if CDi == 1:
        return -1e-8, constr1, constr2, constr3, constr4, constr5, constr6, constr7, constr8, constr9 # AVL failed
    ...
    return -CLCD_return, constr1, constr2, constr3, constr4, constr5, constr6, constr7, constr8, constr9
